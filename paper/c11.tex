...
\begin{align*}
  \begin{gathered}[t]
    ...
    \hbox{\begin{tikzinlinesmall}[node distance=1.5em]
        \event{e}{(v{=}r\lor w{=}r)\limplies (r{=}0\lor r{=}1)\mid\DW{x}{2}}{}
        \event{r}{\DR{y}{v}}{left=of e}
        \phevent{e1}{v{=}r\limplies r{=}0\mid\DW{x}{2}}{below left=1em and -7em of e}
        \phevent{e2}{v{=}r\limplies r{=}1\mid\DW{x}{2}}{below right=1em and -7em of e}
...
\begin{gather*}
  ...
  \hbox{\begin{tikzinline}[node distance=1.5em]
    ...
    \end{tikzinline}}    
\end{gather*}
It is somewhat surprising that the writes are independent of the read!

To understand what has happened, let $\ledelay$ be defined as follows:
\begin{displaymath}
  \fmrg{\bEv}\ledelay\fmrg{\aEv} \textif \bEv\xpox\aEv \textand \labeling(\bEv)\rdelays \labeling(\aEv).
\end{displaymath}
In the example above, $\ledelay$ is a preorder, but not a partial order.
It is possible to rule out such examples by requiring that $\ledelay$ be a
partial order.  This would provide a more precise notion of dependency for
\cXI{}, which is similar to the dependency calculated for \PwTmca{}s.

\todo{Perhaps use a better example:
  \begin{math}
    \IF{r}\THEN\PW{x}{1}\FI
    \SEMI
    \IF{s}\THEN\FENCE{\fSC}\FI    
    \SEMI
    \IF{\BANG r}\THEN\PW{x}{1}\FI    
  \end{math}}
