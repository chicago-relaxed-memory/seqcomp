...
is acyclic.  This is a more precise categorization of thin-air behavior, and 
...
construction (described in the proof of \reflem{lem:po}), program order
($\rpox$) is a pre-order, which may include cycles due to coalescing.  For
example:
\begin{align*}
  % \PR{y}{r}
  % \SEMI
...
\begin{lemma}
  ...
  \begin{proof}
    ...
    same event name for $\aPS'$.  If an event is merged in
...
The following lemma (immediate from \ref{pom-m-kappa}) shows that if $\aPS$ is \emph{complete}, then
...
A pomset in the image of $\fextract{}$ is a \cXI{} \emph{candidate execution}.
%
As an example, consider \labeltext[\textsc{tc6}]{Java Causality Test Case 6}{JC-TC6} \cite{PughWebsite}.  Taking $w=0$ and $v=1$,
...
ignores synchronization and coherence completely.  For example, consider:
\begin{align*}
...
\begin{align*}
  ...
  \end{gathered}
\end{align*}
It is possible to mimic this in \PwTc{}, without introducing extra
...
\eqref{eq5}, %this execution,
it suffices to require that $\ledelay$ is a partial order.

Program \eqref{eq5} shows that the definition of semantic dependency is up for debate in \cXI.
The International Standard Organization's C++ concurrency subgroup acknowledges that semantic
