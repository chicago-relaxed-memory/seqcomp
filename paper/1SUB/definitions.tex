...
\begin{definition}
  ...
  where $\Cdown{\aEv}=\{ \cEv \mid \cEv \lt \aEv \}$, %if $\labeling(\aEv)$ is a write, and $\Cdown{\aEv}=\aEvs_1$, otherwise
  % let
  % \begin{math}
  %   \smash{
...
\begin{enumerate}[label=(\textsc{p}\arabic*),ref=\textsc{p}\arabic*]
  \setcounter{enumi}{7}  
\item[] \labeltext[\textsc{p}7]{}{par-rf-arm}
  \begin{enumerate}[leftmargin=0pt]
...
\begin{displaymath}
  \Cdown{\aEv}=
  \begin{cases}
    \{ \cEv \mid \cEv \lt \aEv \} & textif \labeling(\aEv) \;\text{is a write}
    \\
    \aEvs_1 & \textotherwise
  \end{cases}
\end{displaymath}
...
\begin{enumerate}
\item[(\textsc{s}6c)] \labeltext[\textsc{s}6c]{}{seq-le-blocks}
