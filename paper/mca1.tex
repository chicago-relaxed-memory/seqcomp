...
We derive a model of concurrent computation by adding
% \emph{parallel composition} and 
\emph{reads-from} to \reffig{fig:seq}.  To model coherence
and synchronization, we add \emph{delay} to the rule for sequential
composition.  For \emph{multicopy-atomic} (\mca{}) architectures, it is sufficient to encode delay in
...
discussion of item \eqref{lem:ifelse:if:if} %and \eqref{lem:ifelse:if:if2},
