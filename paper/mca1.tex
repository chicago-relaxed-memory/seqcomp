...
Concerning \reflem{lem:if}:
For \PwTmca{}, \eqref{lem:ifelse:if:if1} and \eqref{lem:ifelse:if:if2} are
inclusions rather than equations.  In \textsection\ref{sec:delay}, we refine
the semantics to validate the reverse inclusions.


\subsection{\PwTmcaTITLE{1}}
\label{sec:mca1}

Parallel composition is disjoint union.  Any $\rrfx$ edges added between the
two sides must also be added to the order (\ref{par-rf-le1} and
\ref{par-rf-le2}).
...
We include the \emph{reads-from} relation explicitly in the model
\eqref{pom-rf}.
