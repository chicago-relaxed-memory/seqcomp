...
After some preliminaries, we define the basic model
(\textsection\ref{sec:pomsets} and \reffig{fig:sem}).  We explain the model
using examples (\textsection \ref{sec:ex1}--\ref{sec:ex3}), establish some
basic properties (\textsection\ref{sec:props}), and discuss program
transformations (\textsection\ref{sec:valid}--\ref{sec:invalid}).  We encourage
...
\subsection{Examples: Pomsets}
\label{sec:ex1}
\subsection{Examples: Pomsets with Preconditions}
\label{sec:ex2}
\subsection{Examples: Pomsets with Predicate Transformers}
\label{sec:ex3}
...
\label{sec:valid}
\subsection{Invalid Transformations}
\label{sec:invalid}
