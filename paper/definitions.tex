...
\begin{example}
  ...
  such as:
    ...
    \hbox{\begin{tikzinline}[node distance=1ex and 1.5em]
      ...
      \end{tikzinline}}
    \end{gather*}
  If $\aVal$ is chosen badly, we have a pomset with a precodition
...
\begin{example}
  ...
  \begin{align*}
    ...
    \end{gathered}
  \end{align*}
  Since there is no requirement for disjointnessin the semantics of conditionals,
  we can can consider the case where the pomsets share their event,
  in which case:
    \begin{gather*}
    ...
    \hbox{\begin{tikzinline}[node distance=1ex and 1.5em]
      ...
      \end{tikzinline}}
    \end{gather*}
  where the precondition on $(\DW{y}{1})$ is a tautlogy, and so is independent of $r$.
\end{example}
...
\begin{definition}
  A pomset with predicate tansformers is a pomset with preconditions (\refdef{def:pomsets-pre}),
