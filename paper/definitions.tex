...
$\aEv$ in causal order ($\bEv\in\Cdown{\aEv}$ when $\bEv<_1\aEv$).  % We
...
two events. Then $\sSEMI{}{}$ requires %$\aVal,\bVal\in\Val$ and
...
\begin{align*}
  ...
  \end{gathered}
\end{align*}
Here we apply the transformer from the left %$\aTr[0]{\emptyset}{}$
to $(x{=}1)$,
...
The semantics validates dead code elimination:
...
\begin{gather*}
  ...
  % \end{aligned}
\end{gather*}
Note that $\PW{x}{0}$ only appears on the untaken branch of the conditional.
...
read.  % The problem here is that we have ignored this write when calculating
...
such a justification---leaves the dependency in place. \todo{Mark/Simon:
  please check this.}
