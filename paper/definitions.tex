...
The definition follows \citet{DBLP:journals/cacm/Dijkstra75}. (See
\textsection\ref{sec:false} for a discussion.)
...
capturing termination.  In $\sSTORE{}{}$, \ref{write-term} ensures that
all writes are included in complete pomsets---note that $\kappaE{\emptyset} = \FALSE$.  This also ensures
...
following example, where we elide transformers for the writes and ``$\BANG$''
