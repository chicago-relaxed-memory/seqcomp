...
entail the transformer of a larger set.  In sequential composition, adding
more order can only increase the size of ${\Cdown{\aEv}}{}$; thus, larger
dependent-sets will always satisfy more preconditions.
(Note that the definition is written to be insensitive to events outside $\aEvs$.)
...
In addition, \ref{write-term-nonempty} ensures that complete pomsets do not
...
event at top-level, use the empty set.
...
\begin{math}
  \sembase{\PW{x}{2}}
\end{math}
cannot include a pomset with event $\DWP{x}{3}$, since $2{=}3$ is not a
...
There are two aspects of \reffig{fig:seq} that are necessary to cover corner cases.

First, consider local invariant reasoning, as in \jmm{} causality test case 1
\cite{PughWebsite}:
\begin{gather*}
  \begin{gathered}
    \PW{x}{0} \SEMI
...
Second, consider $\Q{x}$, which we motivate using the following example 
...
\begin{align*}
  \begin{gathered}    
    ...
    \hbox{\begin{tikzinline}[node distance=1.5em]
        ...
        \event{a5}{\aForm\mid\DW{z}{1}}{right=of a4}
...
$(1{=}r\limplies r{\neq}0)$, which is a tautology, and the execution is
...
With $\Q{x}$, the precondition $\aForm$ is
$(1{=}r\limplies (r{\neq}0 \land \Q{x}))[\FALSE/\Q{x}]$, which is
unsatisfiable.  Intuitively, $\Q{x}$ requires that the most recent
prior write to $\aLoc$ must be in the pomset in order to read $\aLoc$.
...
\ref{write-tau-nonempty} and \ref{write-tau-empty} respectively.
...
written using substitution \cite{DBLP:journals/pacmpl/JagadeesanJR20}.
...
% \begin{align*}
  ...
  \end{gathered}
\end{align*}
The precondition $\aForm=(1{=}0\land y{=}0)\lor (1{\neq}0 \land y{\neq}0)$,
...
uniquely chosen registers.  Using \reffig{fig:seq}, we compute a tautology in
the example above: $\aForm=((y{=}r\lor1{=}r)\limplies r{=}0)\lor (r{\neq}0)$.
