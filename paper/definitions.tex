...
\begin{gather*}
  ...
  \end{gathered}
\end{gather*}
The execution is allowed due to the evident cycle.  All of the order shown is
...
coherence is weaker than C11, to support common subexpression elimination,
and stronger than Java, to support local reasoning about data races.  See
...
capturing termination.  Roughly, the structural rules take $\aTerm$ to be
$\aTerm_1\land\aTerm_2$.  The interesting rules are $\sLOAD{}{}$,
$\sFENCE{}$, and $\sSTORE{}{}$.
...
In top-level pomsets, \ref{top-kappa} requires that every precondition $\labelingForm(\aEv)$ is a tautology,
...
is introduced between events in $\aEvs_1\cap\aEvs_2$.\footnote{This ensures
  ...
  \end{math}
  does not include 
  \begin{tikzinlinesmall}[node distance=1.5em]
    \event{a}{\DR{x}{1}}{}
    \event{b}{\DW{x}{1}}{right=of a}
    \rf[out=170,in=10]{b}{a}
    \wk{a}{b}
  \end{tikzinlinesmall}, taking $\xrfx$ from the left and $\le$ from the right.}
