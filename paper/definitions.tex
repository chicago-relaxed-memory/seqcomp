...
\emph{Memory references}, aka \emph{locations}, are tagged values, written $\REF{\cVal}$.  Let $\Loc$
...
\item expressions do \emph{not} include memory references:
  $\aExp[\bExp/\aLoc]=\aExp$ (for all $\aLoc$).
\end{itemize}
...
\citet{DBLP:journals/cacm/Dijkstra75}.\footnote{In \label{footnote:false}
  ...
  \labeltext[\textsc{x}4$'$]{(\textsc{x}4$'$)}{tr-false-footnote'}
  ...
  We can then require \labeltext[\textsc{x}4]{(\textsc{x}4)}{tr-false-footnote}
...
$\aEv$ in causal order ($\bEv<_1\aEv$ implies $\bEv\in\Cdown{\aEv}$).  % We
% call $\Cdown{\aEv}$ the \emph{dependent set} of $\aEv$, and
% $\aEvs\setminus\Cdown{\aEv}$ the \emph{independent set}.
Under the following definition, the larger ${\Cdown{\aEv}}{}$ is, the
...
Throughout the rest of this section, we endeavor to explain \reffig{fig:seq},
...
$3$ always refers to $\labelingForm$ and item $5$ always refers to $\aTerm$.
...
the definition of $\labelingForm'_2$ in $\sSEMI{}{}$ in \reffig{fig:seq},
...
% As with \PwP{}, the empty pomset is in the semantics of every
% program---the termination condition prevents $\SKIP$ from being a refinement of
% $\PW{x}{1}$.
...
have verified \reflem{lem:monoid} and \reflem{lem:if}\ref{lem:if:seq} in
Coq\footnote{Specifically, we have proven these results for the semantics of
  \reffig{fig:seq} with the refinements of \textsection\ref{sec:read-read},
  \textsection\ref{sec:semreg}, and \textsection\ref{sec:semca}}.  Similar
laws apply to parallel composition---for example
$\sem{\aCmd}=\sem{\SKIP\RPAR\aCmd}$.  Note, however, that
$\sem{\aCmd}\neq\sem{\aCmd\RPAR\SKIP}$---this asymmetric operator throws
...
well as inclusion (see the definition of $\rextendsdef{}{}$ in \reffig{fig:seq}).
...
The semantics is closed with respect to {augmentation:} $\aPS_2$ is an
...
Augment closure captures the intuition that it
...
\emph{augment-minimal}. % In examples, we typically
...
Ignoring the logic, the definitions of \reffig{fig:seq} are straightforward.
...
In complete pomsets, \ref{top-kappa} requires that all preconditions must be
tautologies.  In order to allow complete pomsets with untaken conditionals,
such as $\IF{\FALSE}\THEN\PW{x}{1}\FI$, we allow the empty pomset in the
semantics of all statements.  % To ensure that $\SKIP$ is not a refinement of
% $\PW{x}{1}$,
...
conditional.
% \ref{read-term}
% requires that acquiring reads be included.  %This also ensures that
...
For reads, the definition of $\aTerm$ depends on the mode: relaxed reads may
...
\begin{align*}
  \begin{gathered}
    ...
    \hbox{\begin{tikzinline}[node distance=.5em and 1.5em]
        \event{a}{\DW{x}{1}}{}      
        \event{c}{\aExp{\neq}0\bigmid\DW{x}{3}}{right=of a}
        %\wki{a}{c}
      \end{tikzinline}}    
  \end{gathered}
\end{align*}
However, this pomset is incomplete---regardless of $\aExp$---since $\aTerm\riff2{=}3\riff\FALSE$.
...
predicate transformers\allowbreak{}---$\hbox{}\,\aTr[1]{}{}$---from the left to
preconditions---$\labelingForm_2(\aEv)$---on the right.  The specific rule is
\ref{seq-kappa},
...
nontrivial preconditions and predicate transformers are introduced.  
\begin{multicols}{2}
...
The rules track dependencies.    
We discuss data dependencies \eqref{write-kappa} here and control
dependencies \eqref{if-kappa} in \textsection\ref{sec:ex:control}.  We enrich
the semantics to handle address dependencies in \textsection\ref{sec:addr}.

A simple example of a data dependency is a pomset
$\aPS\in\sem{\PR{x}{r}\SEMI \PW{y}{r}}$.  If $\aPS$ is complete, it must have
two events. Then $\sSEMI{}{}$ (\reffig{fig:seq})
requires %$\aVal,\bVal\in\Val$ and
$\aPS_1\in \sem{\PR{x}{r}}$ and $\aPS_2\in \sem{\PW{y}{r}}$ of the following
form. (We only show the independent transformer for writes---ignoring
$\Q{x}$, the dependent and independent transformers for writes are the same.)
\begin{align*}
  \tag{$\dagger$}\label{eq2}
  \begin{gathered}
    \PR{x}{r} 
...
\begin{align*}
  ...
  \end{gathered}
\end{align*}
Here we apply the transformer from the left $(\bForm[1/x])$ %$\aTr[0]{\emptyset}{}$
to $(x{=}1)$,
...
% In addition to calculating dependencies, preconditions also determine which
% events can appear in a pomset, since complete pomset must have tautological
% preconditions (\ref{top-kappa}).  For example, any pomset in 
% \begin{math}
%   \sem{
%     \PW{x}{1}\SEMI\PW{y}{1}
%     \PAR  
%     \IF{\PR{x}{}}\THEN\PR{y}{r}\FI
%   }
% \end{math}
% that includes a read of $y$ must also include a read of $x$.
...
Control dependencies are introduced by the conditional. 
% For
% events from $\aEvs_2$, it %\ref{if-kappa}
% requires $\lnot\aForm\land\labelingForm_2(\aEv)$.
...
When events coalesce, \ref{if-kappa} ensures that control
...
\begin{gather*}
  ...
  \end{gathered}
\end{gather*}
% The precondition on the conditional write to $y$ is $r{\geq}0$.
...
predicate transformers from $\sLET{}{}$ or $\sLOAD{}{}{}$ (\reffig{fig:seq}), logical formulae
...
branch of a conditional, creating \emph{indirect dependencies}.
% Of course, if we track local state incorrectly, bad things can happen, such
% as a failure of \drfsc, which requires that properly synchronized programs
% only exhibit SC behaviors.
...
\begin{gather*}
  ...
  \end{array}
\end{gather*}
In SC executions, the left thread always takes the then-branch of the
conditional, reading $0$ for $x$ and therefore not writing $z$.  As a result
the second thread does not write $y$, and the program is data-race-free under
SC.  To satisfy the \drfsc{} theorem, no other executions should be possible.
%
Complete executions of the left thread that take the then-branch must include
$\DWP{x}{0}$, whereas those that take the else-branch must \emph{not} include
$\DWP{x}{0}$.  A problem arises if events from the subsequent code of the
left thread---common to the two branches---coalesce, thus removing an essential
control dependency.  Consider the following candidate execution:
\begin{gather*}
  \tag{$\dagger\dagger$}\label{eq:666}
  \hbox{\begin{tikzinline}[node distance=1.5em]
      \event{a1}{\DW{x}{1}}{}
    ...
    \end{tikzinline}}
\end{gather*}
Note that the write to $z$ depends on the read of $x$, but not the read of
$y$.  Ignoring $\Q{x}$, as we have done up to now, the precondition $\aForm$
is:
\begin{gather*}
  % \aForm \riff
  % (1{=}r \lor y{=}r)
  ...
  \end{aligned}
\end{gather*}
Since $(1{=}s)$ implies $(s{\neq}0)$, the precondition is a tautology and
\eqref{eq:666} is allowed, violating \drfsc{}.

Without $\Q{x}$, the semantics enforces $\DWP{z}{1}$'s direct dependency on
$\DRP{x}{1}$, but not its \emph{indirect} dependency on $\DRP{y}{1}$.  By
eliding $\DWP{x}{0}$, we have forgotten the local state of $x$ in the untaken
branch of the execution.  Nonetheless, we are using the
subsequent---\emph{stale}---read of $x$, by merging it with the read from the
taken branch.  This \emph{half-stale} merged read is then used to justify
$\DWP{z}{1}$.
%the merged write to $z$.
% The problem here is that we have ignored this write when calculating
% the dependency between $\DR{y}{1}$ and $\DW{z}{1}$.

In \reffig{fig:seq}, \ref{read-tau} corrects this by introducing quiescence
symbols into predicate transformers. Quiescence symbols capture the intuition
that---in the untaken branch of a conditional---the value of a read from $x$
can only be used if the most recent local write to $x$ is included in the
execution.  Quiescence symbols are eliminated from formulae by the closest
preceding write (\ref{write-tau}).  With quiescence, the precondition of
\eqref{eq:666} becomes the following:
\begin{displaymath}
  \aForm'\riff
...
Regardless, $\aForm'$ is non-tautological: in the top conjunct, we have lost
the ability to use $1{=}s$ to prove $s{\neq}0$.  Intuitively, $\Q{x}$ is true
when the local state of $x$ is up to date, and false when it is stale.  In
order to read $\aLoc$, $\Q{x}$ requires that the most recent prior write to
$\aLoc$ must be in the pomset.

We also include quiescence symbols directly in preconditions of reads
\eqref{read-kappa}.  This guarantees initialization in complete pomsets:
every $\DRP{x}{}$ must have a sequentially preceding $\DWP{x}{}$ in order to
eliminate the precondition $\Q{x}$.
...
% allowed, resulting in a violation of \drfsc.  
...
% be eliminated by taking $\aEvs=\emptyset$.
...
\begin{gather*}
  ...
  \hbox{\begin{tikzinline}[node distance=.5em and 1.5em]
    ...
    \end{tikzinline}}    
\end{gather*}
\PwT{} correctly allows this execution; \MRD{} forbids it by requiring
$\DRP{x}{1}\xpo\DWP{y}{1}$. The co-product mechanism in \MRD{} seeks an
isomorphic justification under the $\DRP{x}{2}$ branch of the read in the
event structure, and---failing to find such a justification---leaves the
dependency in place.
...
\reflem{lem:monoid}---in particular, the need for sequential composition to
be associative.  In this subsection, we give three examples.

First, the predicate transformers we have chosen for \ref{read-tau-dep}
...
\ref{read-tau-ind-p} does not distribute through disjunction \eqref{tr-or},
and therefore is not a predicate transformer.  This is not merely a
theoretical inconvenience: adopting \ref{read-tau-ind-p} would also break
associativity.  Consider the following example, where ``\texttt{!}''
...
Associating to the right, we 
coalesce the writes then prepend the read:
\begin{align*}
  \begin{gathered}[t]
    \PR{y}{r}
    \\
...
\begin{math}
  (1{=}0\land y{=}0)\lor (1{\neq}0 \land y{\neq}0),
\end{math}
which is not a tautology. %---it is equivalent to $(y{\neq}0)$.
% This pomset can
...
uniquely chosen registers.  Using Skolemization, \reffig{fig:seq} computes
\begin{math}
  \aForm'\riff
...
Second, \citeauthor{DBLP:journals/pacmpl/JagadeesanJR20} impose \emph{consistency}, which requires that for every pomset
