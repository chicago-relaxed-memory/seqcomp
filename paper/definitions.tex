...
\textsection\ref{sec:ex:control}.   % We do not discuss
...
two events. Then $\sSEMI{}{}$ (\reffig{fig:seq}) requires %$\aVal,\bVal\in\Val$ and
...
Control dependencies are introduced by the conditional. For events from
$\aEvs_1$, \ref{if-kappa} requires $\aForm\land\labelingForm_1(\aEv)$. For
events from $\aEvs_2$, it %\ref{if-kappa}
requires $\lnot\aForm\land\labelingForm_2(\aEv)$.  For coalescing events in
$\aEvs_1\cap\aEvs_2$, it %\ref{if-kappa}
requires
...
\reffig{fig:seq} without $\Q{x}$, as we have done up to now, the following
