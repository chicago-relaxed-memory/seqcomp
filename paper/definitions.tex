...
\begin{definition}
  ...
  \item \label{pom-le} \makecounter{le}
    ...
    \item \label{pom-le-match}
      if $\bEv\xrfx\aEv$ then either $\bEv\le\bEv$ or $\aEv\le\bEv$,
...
\begin{lemma}
  ...
  \begin{math}
    \xIFTHEN{\aForm}{
      \xIFTHEN{\bForm}{\aPSS_1}{}
    }{}
    =
    \xIFTHEN{\aForm\land\bForm}{\aPSS_1}{}
  \end{math}
  ...
  \begin{proof}
    Straightforward calculation.  Associativity of $\SEMI\!$ requires both
    conjunction closure \eqref{tr-and} (for termination) and disjunction
    closure \eqref{tr-or} (for the predicate transformers themselves).  Right
    unit of $\sSKIP$ for $\SEMI\!$ requires \ref{pom-term-tau}.
  \end{proof}
\end{lemma}
\begin{definition}
  \label{def:augment}
...
Consider the following, eliding transformers for the writes:
\begin{align*}
  \begin{gathered}[t]
    \PR{y}{r}
    \\
