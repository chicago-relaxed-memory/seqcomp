...
\item there are binary relations $\rmatchesdef$, $\rblocksdef$, and
  $\rdelaysdef$ between actions,
\item there is a subset of actions, known as $\sreleasedef$ actions.
\end{itemize}
...
where $\aEvs_1$ and $\aEvs_2$ are not necessarily disjoint.  We say $\aRel$
\emph{extends} $\aRel_1$ and $\aRel_2$ if
...
\begin{definition}
  ...
  \item[] \stepcounter{enumi}\label{par-rf}
    ...
    \item \label{seq-rf3}
      if $(\bEv,\aEv)\in{\rrfx}$
      and $\labeling_1(\cEv)$ blocks $\labeling_2(\aEv)$
      then  $\cEv\le\bEv$,
    \end{enumerate}
