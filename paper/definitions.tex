...
The definition of $\rextendsdef{}{}$ ensures that no new order is introduced
between events in $\aEvs_1\cap\aEvs_2$.\footnote{For example, using the
  semantics of \textsection\ref{sec:arm2}, \ref{if-rf-extends} ensures that
\begin{math}
...
% Many laws hold for the conditional, such as dead code elimination
...
Many laws hold for the conditional.  For example:
\begin{align*}
   \sem{\IF{\aExp}\THEN\aCmd\ELSE\aCmd\FI} &\supseteq
   \sem{\aCmd}
 \end{align*}
We discuss the inverse inclusion in \textsection\ref{sec:semca}.
