...
\subsection{Arm Compilation}

% Arm does not enforce read-read control dependencies.
% So we need to weaken the definition of
% \item
%   Change the definition of $\Cdown{\aEv}$:
...
The model compiles correctly to arm using the lowering:
\begin{align*}
...
\begin{itemize}
\item%[(\textsc{s}6c)] \labeltext[\textsc{s}6c]{}{seq-le-rf}
  Replace \eqref{seq-le-rf} with:
  if $\labeling_1(\cEv)$ blocks $\labeling_2(\aEv)$
  then $\bEv\xrfx\aEv$ implies $\cEv\le\bEv$.
\item
  Replace $\eqreorderco$ by $\reorderlws$ in \refdef{def:actions} of
  $\rdelaysdef$, where
  \begin{math}
    {\reorderlws}
    =
...
If one wants a post-hoc verification technique for $\rrfx$, it is possible to
...
% \begin{enumerate}
% \item[(\textsc{m}7d)]
%   if $\labeling(\cEv)$ blocks $\labeling(\aEv)$
%   then $\bEv\xrfx\aEv$ implies $\cEv\le\bEv$.
%   %   if $\bEv\xrfx\aEv$ and $\labeling(\cEv)$ blocks $\labeling(\aEv)$ then not $\bEv\le\cEv\xpox\aEv$.
% \end{enumerate}
