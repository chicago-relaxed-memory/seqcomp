...
\item formulae include symbols % for all $\aLoc$,
  ...
  $\Q{\mSC}$, $\Q{\mRA}$, $\Qx{\aLoc}$, 
  $\Qw{\aLoc}$, $\Dx{\aLoc}$, $\RW$, % for all $\aLoc$,
...
This use of independency is inspired by
\citet{DBLP:books/ws/95/Mazurkiewicz95}.  It is worth noting that if
${\reorder}$ is taken to be the empty relation, then fulfilled pomsets in the
following semantics correspond to sequentially consistent executions
\cite{Lamport:1979:MMC:1311099.1311750} up to mumbling
...
\begin{definition}%$\phantom{\;}$\par
  ...
  \item[\ref{par-E}--\ref{par-disjoint})]
    as for $\sPAR$ in \refdef{def:pomsets},
  ...
  \item[\ref{pre-E}--\ref{pre-reorder})]
    as for $\DWP{\aLoc}{\aVal}\sPREFIX\aPSS_2$ in \refdef{def:pomsets}, % (ignoring \thepomsetPrefixOrderCount),
  ...
  \begin{enumerate}
    \setcounter{enumi}{\value{pomsetPrefixOrderCount}}
  \item[\ref{pre-E}--\ref{pre-reorder})]
    as for $\DRP{\aLoc}{\aVal}\sPREFIX\aPSS_2$ in \refdef{def:pomsets}, % (ignoring \thepomsetPrefixOrderCount),
...
\refdef{def:pomsets-trans}, below, which captures dependency.  In
...
\begin{figure*}
  ...
  \caption{Full Semantics of Loads, Stores and Threads (See \refdef{def:QS}
    for $\QS{}{}$/$\QL{}{}$ and \refdef{def:DS} for $\DS{}{}$/$\DL{}{}$)}
  \label{fig:full}
\end{figure*}    
