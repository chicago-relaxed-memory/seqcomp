...
\begin{align*}
  ...
  % \BNFSEP \PA{\aLoc}{\aExp} 
  \\[-.5ex]
  \BNFSEP& \aCmd_1 \RPAR \aCmd_2 %\FORK{\aGrp}
...
% % We often drop the words $\FORK{}$ and $\THREAD{}$.
...
\item formulae include quiescence symbols $\Qsc$, $\Qr{\aLoc}$, $\Qw{\aLoc}$
  (used in \textsection\ref{sec:q}), the downgrade symbols $\Dx{\aLoc}$
  (used in \textsection\ref{sec:downgrade}), and the write symbol $\RW$
  (used in \textsection\ref{sec:rrd}),
...
\begin{definition}
  \showRAtrue
  \label{def:reorderra}
  Reorderability relations.
  Coherence-after and synchronization.
  \begin{align*}
    {\reorderca}
    &=
    \{(\DW{\aLoc}{}, \DW{\bLoc}{}) \mid \aLoc\neq\bLoc\}
    ...
    \cup\{(\DF{\mREL},      \DF{\mACQ}    ) \}
    % \\&
    % \cup\{(\DR[\mRLX]{\aLoc}{}, \DG{\mACQ}{\aLocs})\mid \aLoc \in\aLocs \}
    \\
    {\reorderlt}
    &=
    {\reorderra}\cap{\reorderca} %\{(\aAct, \bAct) \mid \aAct\reorderco\bAct \land \aAct\reorderra\bAct\}
  \end{align*}  
\end{definition}
...
\begin{definition}
  ...
  is a tuple $(\aEvs, \labeling, \labelingForm, \aTr{}{}, {\rrfx}, {\le})$ where
  \begin{enumerate}
  ...
  \item
    \label{pom-rf}
    ${\rrfx} \subseteq (\Event\times\Event)$ is a relation capturing \emph{reads-from} such that %$\{(\bEv,\aEv)\mid\labeling(\aEv)=\DWP{\aLoc}{\aVal}\textand\labeling(\bEv)=\DRP{\aLoc}{\aVal}\}$,
    \begin{itemize}
    \item[(\ref{pom-rf}a)]
...
\begin{definition}
  % \noindent
  If $\aPS \in \sRPAR{\aPSS_1}{\aPSS_2}$ then  
  ...
  \begin{enumerate}[topsep=0pt]
    \setcounter{enumi}{3}
  \item[(\ref{par-E}--\ref{par-rf})] as for $\sRPAR{}{}$,
  \item
  ...
  \item[(\ref{par-E}--\ref{par-rf})] as for $\sRPAR{}{}$,
  \item
    ${\rrfx}\subseteq\PBR{{\rrfx_1}\cup{\rrfx_2}\cup(\aEvs_1\times \aEvs_2)}$, 
  \item
    if $(\bEv,\aEv)\in{\rrfx}$
    and $\labeling(\cEv)$ blocks $\labeling(\aEv)$, 
    where $\cEv\in\aEvs_1$ and $\aEv\in\aEvs_2$, 
    then either $\cEv\le\bEv$ or $\aEv\le\cEv$.
  ...
  \item \label{seq-reorder}
    if $\bEv\in\aEvs_1$ and $\aEv\in\aEvs_2$ then either $\bEv\le\aEv$ or $\labeling_1(\bEv)\reorderlt\labeling_2(\aEv)$.
  \end{enumerate}
  \medskip
  ...
  \begin{multicols}{2}
    If $\aPS \in \sSTORE[\amode]{\aLoc}[\ascope]{\aExp}$ then
    $(\exists\aVal\in\Val)$
    \begin{enumerate}[topsep=0pt]
    \item
     $\aEvs\neq\emptyset$ and if $\bEv,\aEv\in\aEvs$ then $\bEv=\aEv$,
    \item
      $\labelingAct(\aEv) = \DW[\amode]{\aLoc}[\ascope]{\aVal}$,
    \item
    ...
    \end{enumerate}
    \columnbreak
    If $\aPS \in \sLOAD[\amode]{\aReg}[\ascope]{\aLoc}$ then
    $(\exists\aVal\in\Val)$
    \begin{enumerate}[topsep=0pt]
    \item
      if $\bEv,\aEv\in\aEvs$ then $\bEv=\aEv$,
    \item
      $\labelingAct(\aEv) = \DR[\amode]{\aLoc}[\ascope]{\aVal}$
    % \item
    %   $\labelingForm(\aEv)$ implies
    %   \begin{math}
    %     \TRUE
...
% Given these definitions, the semantics of commands is:
  \begin{scope}
    ...
    \begin{gather*}
      ...
      \\
      \begin{aligned}
        \sem{\PW[\amode]{\aLoc}{\aExp}} &= \sSTORE[\amode]{\aLoc}[\ascope]{\aExp}
        &
        \sem{\SKIP} &= \sSKIP 
        \\
        \sem{\PR[\amode]{\aLoc}{\aReg}} &= \sLOAD[\amode]{\aReg}[\ascope]{\aLoc}
        &
        \sem{\aCmd_1 \RPAR \aCmd_2} &= \sRPAR{\sem{\aCmd_1}}{\sem{\aCmd_2}}
        \\
        \sem{\LET{\aReg}{\aExp}} &= \sLET{\aReg}{\aExp}
        & 
        \sem{\aCmd_1 \SEMI \aCmd_2} &= \sSEMI{\sem{\aCmd_1}}{\sem{\aCmd_2}}
        \\
        \sem{\PF[\ascope]{\amode}} &= \sFENCE[\ascope]{\amode}
        &
      \end{aligned}
    \end{gather*}
  \end{scope}
...
\medskip
  ...
  \item
    $\labelingAct(\aEv) = \DW[\amode]{\aLoc}[\ascope]{\aVal_\aEv}$,
  \item
    $\labelingForm(\aEv)$ implies
    \begin{math}
      \cForm_\aEv
  ...
  %   \end{math}
    ...
    %   \end{math}.
  \item
    $\bigvee_{\aEv\in\aEvs}\cForm_\aEv$ is a tautology.
  \end{enumerate}
...
\medskip
  ...
  \item
    if $\cForm_\bEv\land\cForm_\aEv$ is satisfiable then $\bEv=\aEv$,
  \item
    $\labelingAct(\aEv) = \DR[\amode]{\aLoc}[\ascope]{\aVal_\aEv}$
  \item
    $\labelingForm(\aEv)$ implies
    \begin{math}
      \cForm_\aEv
  ...
  \item
    ...
    \end{math},
\item 
  if $\bigvee_{\aEv\in\aEvs}\cForm_\aEv$ is unsatisfiable then  
  \begin{math}
    (\forall\bReg)
  \end{math}
  $\aTr{\bEvs}{\bForm}$  implies 
...
\begin{itemize}
\item \makebox{$\aEv\xrf\bEv$} arises from $\rrfx$, where $\aEv\lestrong\bEv$,
\item \makebox{$\aEv\xrfint\bEv$} arises from $\rrfx$, where $\lnot(\aEv\lestrong\bEv)$.
\end{itemize}
