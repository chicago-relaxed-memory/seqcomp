...
This idea is borrowed from \cite{DBLP:journals/pacmpl/JagadeesanJR20}, which
includes further examples.  See \textsection\ref{sec:ex:assoc} for further
...
which is not a tautology.  This pomset can never be complete due to the bogus
write of $(\PW{x}{\BANG r})$, which will show up in the termination condition $(1{=}0)$.
Nevertheless, this is a problem, since associativity must reckon with
incomplete pomsets.

Our solution is to Skolemize, replacing substitution by implication, for 
% uses of $\bForm[\aVal/\aReg]$
% by $(\aReg{=}\aVal)\limplies\bForm$,
uniquely chosen registers.  Using \reffig{fig:seq}, we compute
$\aForm=((y{=}r\lor1{=}r)\limplies r{=}0)\lor (r{\neq}0)$ in the example
above, which is a tautology.
