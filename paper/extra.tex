\section{Additional Examples (\PwTmcaTITLE{})}
\label{sec:extras}
...
We presented a thin-air behavior involving address calculation in
\textsection\ref{sec:addr}.  \ref{TC12} provides another example.  The
justification for \ref{TC12} states:
\begin{quotation}
...
Note that \ref{TC12} really isn't about address calculation.  The same
example can be played out with 
control dependencies.  (We elide initializing writes, all $0$):
\begin{gather*}
...
\citet[\textsection6]{DBLP:journals/pacmpl/JagadeesanJR20} provide the
...
Here are some additional examples from \cite{DBLP:journals/pacmpl/JagadeesanJR20}:
\begin{gather*}
  \taglabel{TC8}
  \begin{gathered}
    \PW{y}{0}
...
From \cite{DBLP:journals/pacmpl/JagadeesanJR20}:
\begin{gather*}
  \taglabel{Internal1}
  \begin{gathered}
    \PW{x}{1} \SEMI
...
\subsection{Roach Motel: Commuting Release and Acquire}
...
%\subsection{More RMW}
The following examples are from \cite{promising-ldrf}.
...
% with current barrier placement. 
% On power, release-acquire implemented with lwsync
% \begin{verbatim}
% volatile int x, y;
% Thread 1: x = 2; r1 = y // 0
% Thread 2: y = 1
