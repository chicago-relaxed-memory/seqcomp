...
\begin{gather*}
  ...
  \hbox{\begin{tikzinline}[node distance=1.5em]
   ...
   \end{tikzinline}}
\end{gather*}
But we allow the following, which \cite{Dolan:2018:BDR:3192366.3192421}
disallows (do to lack of read-to-write reordering).
\begin{gather*}
  \PR{y}{r}\SEMI \PW{x}{1}
  \PAR
  \PR{x}{s}\SEMI \PW{y}{2}
...
\section{Old Notes}
\subsection{Read-Read Dependencies}
Alan is somewhat obsessed by the worry that lack of read-read dependency is a
problem.
Most recently, he thought it might break external DRF-SC.  But no one can
construct an example.  Non-problem:
\begin{gather*}
