...
In this subsection, we model a variant of our language that removes
...
As noted \textsection\ref{sec:fork}, every pomset in $\sem{\FORK{\aGrp}}$ is
...
model coherence using independency (\textsection\ref{sec:pomsets}), rather
than encoding it in the logic.
\begin{definition}
  \noindent
...
Sequential composition ensures coherence.  Note that this definition is
incorrect if one allows $\FORK{}$ parallelism, since \ref{T3full} substitutes
