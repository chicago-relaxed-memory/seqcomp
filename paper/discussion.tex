...
Dijkstra's predicate transformers do not distinguish thread-local variables
from shared variables.  Thus, the assignment rule applies to all types of storage:
\begin{itemize}
% \item
%   \begin{math}
...
Let $\aState$ and $\rho$ range over substitutions in $(\Reg\cup\Loc)\fun\Val$.
Treating substitutions as states, the big-step operational semantics of
programs can be defined as a relation $(\aState,\aCmd)\Downarrow\bState$.

Suppose $(\aState,\aCmd)\Downarrow\rho$ and $\aForm=\fwp{\aCmd}{\bForm}$.
The standard interpretation of the predicate transformers states that if
$\aForm$ holds in the initial state $\aState$, then $\bForm$ holds in the
final state $\bState$.  Formally, if $\fwp{\aCmd}{\bForm}\aState$ is a
tautology and $(\aState,\aCmd)\Downarrow\rho$ then $\bForm\bState$ is a
tautology.
...
\begin{gather}
  \nonumber
  (r{>}0) = \fwp{\PW{x}{r{+}1}}{x{>}1}
  \\
  \label{wp1}
  ([5/r], (\PW{x}{r{+}1})) \Downarrow [5/r,6/x]
  \\
  \label{wp2}
  ([0/r], (\PW{x}{r{+}1})) \Downarrow [0/r,1/x]
\end{gather}
...
\begin{scope}
  ...
  \item[\ref{L4})]
    $(\forall\dVal)$ $\aTr{\bEvs}{\bForm}$ implies $\cExp{=}\dVal \limplies (\cExp{=}\cVal\limplies\aVal{=}\aReg)\limplies\bForm$, 
  \item[\ref{L5})]
    $(\forall\dVal)$ $\aTr{\cEvs}{\bForm}$ implies $\cExp{=}\dVal \limplies (\aVal{=}\aReg\lor\RW) \limplies \bForm$.
  \end{enumerate}  
    ...
    \begin{gathered}
      ...
      \hbox{\begin{tikzinline}[node distance=.5em and 1.5em]
          ...
          \event{a3}{(r\EQ2\limplies 1\EQ s)
            \limplies s\EQ1\mid\DW{x}{1}}{below right=.5em and -3em of a2}
          \po{a2}{a3}
        \end{tikzinline}}
    \end{gathered}
  \end{gather*}  
