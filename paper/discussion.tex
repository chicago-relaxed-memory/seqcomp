...
[Not sure it is worth working this out in the logical version of
synchronization.]
...
transformer.  For \xRRD{}, \refdef{def:pomsets-rr} states:
\begin{enumerate}
\item[\ref{L4})]
  $\aTr{\bEvs}{\bForm}$ implies $\aVal{=}\aReg\limplies\bForm$, 
\item[\ref{L5})]
...
\item[\ref{L6})] 
  $\aTr{\dEvs}{\bForm}$ implies $\bForm$, when $\aEvs=\emptyset$.
\end{enumerate}
The independency reasoning of \ref{L5} is only applicable for pomsets where
the ignored read is present!
...
Rather than fork-join, we can use left par and take register state from the
left, but quiescence from both sides.  This removes the syntax level of
thread groups.
\begin{align*}
  \aCmd
  \BNFDEF& \ABORT
