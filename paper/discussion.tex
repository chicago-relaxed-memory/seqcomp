...
\begin{align*}
  ...
  &&
  \begin{tikzcenter}
    \node (fsc) at (3, 0) {$\mathstrut\mFSC$};
    \node (rel) at (2, -0.3) {$\mathstrut\mREL$};
    \node (acq) at (2,  0.3) {$\mathstrut\mACQ$};
    \draw[->](rel)to(fsc);
    \draw[->](acq)to(fsc);
  \end{tikzcenter}
\end{align*}
...
following, where two actions \emph{overlap} if they access the same location:
\begin{enumerate}
\item
