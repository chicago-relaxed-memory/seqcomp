...
It would seem reasonable to require that $\aTerm$ be $\labeling$-consistent
in all pomsets.  However, doing so is incompatible with our approach to
untaken conditionals.  Consider $\IF{\FALSE}\THEN\PW{x}{1}\FI$.  In order to
construct the final empty pomset with $\aTerm\riff\TRUE$, we must allow the
intermediate empty pomset with $\aTerm\riff\FALSE$.

\todo{What about $\labelingForm(\aEv)$}
...
\todo{What is the story here for \PwTpo{}?}
...
\todo{Add W from 1SUB 4.2 to get read-read independency.  The write rules
  should mention D---see the agda version of write.}
...
\subsection{Is Coherence/Delay Compatible with If-Introduction and Dead-Write-Removal?}
