...
$(\fmode \!\!\BNFDEF\!\! \mREL \!\BNFSEP\! \mACQ \!\BNFSEP\! \mSC)$ is either
\emph{release}, \emph{acquire}, or \emph{sequentially-consistent}.

We can enrich read and write actions to use these modes.
...
\begin{definition}
  ...
  \begin{align*}
    ...
    &\QL{\aLoc}{\mACQ}&=\Qw{\aLoc}\land \Qr{*}
    \\
    [\aForm/\QS{\aLoc}{\mREL}] &= [\aForm/\Qw{*}]
  ...
  \begin{gather*}
    ...
    \begin{gathered}
      \begin{gathered}[t]
        ...
        \\
        \hbox{\begin{tikzinline}[node distance=.5em and 1.5em]
            \raevent{a}{\Qw{x}\land\Qr{*}\mid\DR[\mRA]{x}{v}}{}
    ...
    \begin{gathered}[t]
      ...
      \\
      \hbox{\begin{tikzinline}[node distance=.5em and 1.5em]
          \scevent{a}{\Qr{*}\land\Qw{*}\land\Qsc\mid\DF{\mREL}}{}
...
\begin{example}
  ...
  \begin{align*}
    ...
    \end{gathered}  
  \end{align*}
  [Todo: check to see if ARM allows the following.  If so, try to fix
  downgrading for acquire fences by dropping order from read to fence when it
  has preceding relaxed write.]
  \begin{align*}
    \begin{gathered}[t]
      \hbox{\begin{tikzinline}[node distance=0.5em and 1.5em]
          \event{a}{\DW{x}{2}}{}
...
\subsection{Fork-Join Parallelism 1}
...
\begin{figure*}[t]
  ...
  \begin{center}
    ...
    \end{minipage}
  \end{center}
  \caption{Simplified Quiescence Semantics without Address Calculation
...
\begin{scope}
  ...
  \begin{center}
    ...
    \end{tabular}
  \end{center}
  [Todo: validate the following table and type in the corresponding definition
  for $\reorderra$, following \refdef{def:reorderra}.]
  ...
  \begin{comment}
    ...
    \end{tabular}
  \end{comment}
  It seems that fences generally commute.  %Except $\DRP{x}{}\DFP{\mACQ}\DFP{\mREL}\DWP{y}{}$
  \begin{center}
    \showRAtrue
    \setlength{\tabcolsep}{4pt}
    \begin{tabular}{c|ccc|ccc|ccc}
      ...
      \hline
      $1^{\text{st}}$
      & $\DR[\mRLX]{}{}$ & $\DR[\mRA]{}{}$ & $\DR[\mSC]{}{}$ & $\DW[\mRLX]{}{}$ & $\DW[\mRA]{}{}$ & $\DW[\mSC]{}{}$ & $\DF{\mREL}$&$\DF{\mACQ}$ &$\DF{\mSC}$\\
      \hline                                                                                                                                                     
      ...
      \hline                                                                                                                                                     
      $\DF{\mREL}$     & \cmark           & \cmark          & \cmark          & \xmark           & \xmark          & \xmark          & \cmark      &\cmark       & \cmark    \\
      $\DF{\mACQ}$     & \xmark           & \xmark          & \xmark          & \xmark           & \xmark          & \xmark          & \cmark      &\cmark       & \cmark    \\
      $\DF{\mSC}$      & \xmark           & \xmark          & \xmark          & \xmark           & \xmark          & \xmark          & \cmark      &\cmark       & \cmark 
    \end{tabular}
  \end{center}
  [Todo. Get table and definition to line up with \textsection\ref{sec:fence}, where not all fences commute.]
  \begin{definition}
    \label{def:reorderra}
    Including coherence, \emph{reorderability} is defined:
    \begin{align*}
      {\reorderra}
      &=   \{(\DR[\mRLX]{}{},  \DR[\bmode]{}{}) \}% \mid \amode=\mRLX\}
      \cup\{(\DW[\amode]{}{}, \DW[\mRLX]{}{} ) \}%\mid \bmode=\mRLX \}
      \\&\cup\{(\DR[\amode]{}{}, \DW[\bmode]{}{}) \mid \amode=\mRLX \land \bmode=\mRLX\}
      \\&\cup\{(\DW[\amode]{}{}, \DR[\bmode]{}{}) \mid \amode\neq\mSC \lor \bmode\neq\mSC\}
      \\&\cup\{(\DF{\amode},     \DF{\bmode}    ) \}
      ...
      \\
      {\reorderlt} &= {\reorderra}\cap{\reorderco} %\{(\aAct, \bAct) \mid \aAct\reorderco\bAct \land \aAct\reorderra\bAct\}
    \end{align*}  
  \end{definition}
  \begin{definition}
    \label{def:QSx}
  ...
  \begin{definition}
    ...
    \item[\ref{L3})] 
      $\labelingForm(\aEv)$ implies $\QL{}{\amode} \land \aExp{=}\aVal$
    \item[\ref{L4})]
      $\aTr{\bEvs}{\bForm}$ implies $\aVal{=}\aReg\limplies\bForm$, 
    \item[\ref{L5})]
      $\aTr{\cEvs}{\bForm}$ implies $\bForm[\FALSE/\Q{}]$.
