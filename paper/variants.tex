...
\begin{gather*}
  ...
  \hbox{\begin{tikzinline}[node distance=1.5em]
    ...
    \end{tikzinline}}
\end{gather*}
This execution is not allowed by the semantics $\sembase{}$ of \reffig{fig:sem}: the precondition of
...
\begin{definition}
  ...
  \begin{enumerate}
  \item[(\textsc{p}8)] (\textsc{s}8)\; (\textsc{i}8)\; ${\rrmw}=\PBR{{\rrmw_1}\cup{\rrmw_2}}$,
  \end{enumerate}
\end{definition}
...
%   \end{align*}
% \end{definition}  
As noted in \textsection\ref{sec:ex:term}, the semantics of \reffig{fig:sem}
validates elimination of irrelevant relaxed reads.  In
\textsection\ref{sec:semreg}, we discussed redundant read elimination.
...
for example, $\PW{x}{1}\SEMI\PW{x}{2}$ can be refined to $\PW{x}{2}$.
...
Inspired by \cite[\textsection6.2]{DBLP:journals/pacmpl/ChakrabortyV19}

$\fmerge{}{}:\Act\times\Act\fun2^{\Act}$ be defined as follows, where
$\bmode\lemode\amode$, using the order on modes from
...
\begin{align*}
  ...
  \end{aligned}
\end{align*}
%  Define $\fmerge{}{}:\Act\times\Act\fun2^{\Act}$ as follows, where
% $\bmode\lemode\amode$, using the order on modes from
% \textsection\ref{sec:actions}.
% \begin{align*}
%   \begin{aligned}[t]
...
%   &&
%   \begin{aligned}[t]
%     \fmerge{\DF[\ascope]{\amode}[\aThrd]}{\DF[\ascope]{\bmode}[\aThrd]} &= \{ \DF[\ascope]{\amode\lubmode\bmode}[\aThrd] \}
%     \\
%     \fmerge{\aAct}{\bAct} &= \emptyset, \textotherwise
%   \end{aligned}
% \end{align*}
% Define $\fmerge{}{}:\Act\times\Act\fun2^{\Act}$ as follows, where $\lubmode$
% is the least upper bound with respect the order on modes from
% \textsection\ref{sec:actions}.
% \begin{align*}
%   \begin{aligned}
...
%   \end{enumerate}
% \end{enumerate}
%For associativity, it is important that merge takes the join of two modes.
\begin{enumerate}[topsep=0pt,label=(\textsc{s}\arabic*),ref=\textsc{s}\arabic*]
...
\item[] \setcounter{enumi}{\value{kappa}} \labeltextX{3}{s}{seq-kappa-merge}
  ...
  \item \label{seq-kappa-merge12}
    ...
    % \end{itemize}
    \begin{itemize}
    \item $\labeling(\aEv)=\labeling_1(\aEv)=\labeling_2(\aEv)$ and $\labelingForm(\aEv) \riff \labelingForm_1(\aEv)\lor\labelingForm'_2(\aEv)$,
    \item $\labeling(\aEv)=\labeling_1(\aEv)\neq\labeling_2(\aEv)$ and
      $\labelingForm(\aEv) \riff \labelingForm_1(\aEv)$ and
      $\labelingForm_2'(\aEv) \riff \labelingForm_1(\aEv)$,
    \item $\labeling(\aEv)=\labeling_2(\aEv)\neq\labeling_1(\aEv)$ and
      $\labelingForm(\aEv) \riff \labelingForm'_2(\aEv)$ and 
      $\labelingForm_1(\aEv) \riff \labelingForm'_2(\aEv)$.
      %if $\labeling_1(\aEv)$ is a write then $\labelingForm_1(\aEv) \riff \labelingForm'_2(\aEv)$.
    \end{itemize}
  \end{enumerate}
\end{enumerate}
% (For $\labelingForm$, we could also take the predicate of the ``winner'',
% assuming it is implied by the ``loser''.)
...
You can't just take the precondition of the winner, but also don't need to preserve the condition of the loser.
