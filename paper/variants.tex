...
Test Cases 17--18 \citep{PughWebsite} also require coalescing of reads.
...
referenced by \citet[\textsection6.4]{DBLP:conf/esop/PaviottiCPWOB20}.
...
%We give the semantics of \RMW{}s without address calculation or if-introduction.
...
% \RMW{} operations are more complex.  
To support \RMW{}s, we extend the syntax:
\begin{align*}
  \aCmd
  \BNFDEF& \cdots 
...
supports lowering to Arm load/store exclusive operations.  See
\cite{DBLP:journals/pacmpl/JagadeesanJR20} for examples.
...
\citet{DBLP:conf/pldi/LeeCPCHLV20}, using $\sLOAD{}{}$ rather than $\sLOADP{}{}$.
\begin{scope}
  \small
\begin{gather*}
  \begin{gathered}
...
to $\DWP{y}{1}$ in the last thread.  To see why, consider the independent
...
\subsection{If-Introduction (aka Case Analysis)}
\label{sec:semca}

In order to model sequential composition, we must allow inconsistent
predicates in a single pomset, unlike \PwP{}.  For example, if
...
%\todo{Check definitions and examples in this subsection.}
...
Consider the following program, from \citet[\textsection5]{DBLP:journals/pacmpl/JagadeesanJR20}, where initially $x\EQ0$, $y\EQ0$, $\REF{0}\EQ0$,
...
%     \\
%     \hbox{\begin{tikzinline}[node distance=.5em and 1.5em]
%       \event{b}{r\EQ2\bigmid\DR{\REF{2}}{1}}{}
...
%     \hbox{\begin{tikzinline}[node distance=.5em and 1.5em]
%       \event{a1}{\DR{y}{2}}{}
%       \event{a2}{(2{=}r\lor\RW)\limplies r\EQ2\bigmid\DR{\REF{2}}{1}}{right=of a1}
%       \event{a3}{(2{=}r\lor\RW)\limplies (r\EQ2\limplies 1\EQ s)
%       \limplies s\EQ1\bigmid\DW{x}{1}}{below right=.5em and -8em of a2}
%       \po{a2}{a3}
%     \end{tikzinline}}
%   \end{gathered}
% \end{gather*}  
...
%     \hbox{\begin{tikzinline}[node distance=.5em and 1.5em]
%       \event{a1}{\DR{y}{2}}{}
%       \event{a2}{(2{=}r\lor\FALSE)\limplies r\EQ2\bigmid\DR{\REF{2}}{1}}{right=of a1}
...
%     \hbox{\begin{tikzinline}[node distance=.5em and 1.5em]
%       \event{a1}{\DR{y}{2}}{}
%       \event{a2}{2{=}r\limplies r\EQ2\bigmid\DR{\REF{2}}{1}}{right=of a1}
...
%       \limplies s\EQ1\bigmid\DW{x}{1}}{below right=.5em and -5em of a2}
...
\subsection{Combining Address Calculation and If-Introduction}
\label{sec:semcaaddr}
...
%, using \refdef{def:semaddr}.
%
\refdef{def:semcaaddr} enables this execution using if-introduction.  Under this
semantics, we have:
\begin{align*}
  \begin{gathered}
    \PW{\REF{r}}{0}
...
\refdef{def:semcaaddr} is a mash-up of the \refdef{def:semca} and \refdef{def:semaddr}.
\begin{definition}
  \label{def:semcaaddr}
