...
(\textsection{sec:semca}), software models must not distinguish control
...
% \end{gather*}
%
First we define $\cForm_\labeling$, which captures the register state of a pomset:
\begin{displaymath}
  \cForm_\labeling=
  \textstyle\bigwedge_{\{(\aEv,\aVal)\in(\aEvs\times\Val)\mid\labeling(\aEv)=\DRP{}{\aVal}\}}(\uReg{\aEv}{=}\aVal)
  \textwhere \aEvs=\fdom(\labeling)
\end{displaymath}
We say that $\aForm$ is \emph{$\labeling$-consistent} if $\aForm\land\cForm_\labeling$ is satisfiable.
...
\begin{definition}
  ...
  \begin{enumerate}
  \item[(\textsc{p}10)] (\textsc{s}10)\; (\textsc{i}10)\; ${\rrmw}=\PBR{{\rrmw_1}\cup{\rrmw_2}}$,
  \end{enumerate}
\end{definition}
