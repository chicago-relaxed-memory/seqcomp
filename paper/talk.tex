...
\begin{frame}
  \sol{They're the same!}

  %\sol{MCA!}
  % \solPosR[1cm]{Concurrency is easy!}
  % \probPosR[-1cm]{Sequentiality is hard!}
  %\probX{Power?}  
  %\probX{Control Independencies?}
  %\sol{\begin{tabular}{c}Dependencies\\for C11!\end{tabular}}
...
\begin{frame}
  \frametitle{Revert to 1999 \ldots}
  \onslide<2->A model of imperative programming
  \begin{itemize}
  \item<3-> Shared-memory concurrency 
  \item<4-> Compositional for both parallelism and sequencing 
...
\begin{frame}
  ...
  \onslide<4->
  \vspace{1cm}
  $\DR{x}{v}$ is \emph{fulfilled} if: 
  \begin{itemize}
  \item $\exists\DW{x}{v}<\DR{x}{v}$
  \item \visible<5->{$\forall\DW{x}{u}$ either $\DW{x}{u}\leq\DW{x}{v}$ or $\DR{x}{v}<\DW{x}{u}$}
  \end{itemize}
  \onslide<7->
...
\begin{frame}
  ...
  \only<5->{\probX{How?}}
  \only<6->{\sol{\begin{tabular}{c}Pomsets \\+ Preconditions!\end{tabular}}}
\end{frame}
...
\begin{frame}
  ...
  \large
  Order includes (in addition to dependency):
  \begin{itemize}
...
% \end{frame}
\begin{frame}
  \frametitle{Merging and Synchronized-Before?}
  \huge
  \begin{displaymath}
    \IF{r}\THEN
    \PW{x}{1}
...
\begin{frame}
  ...
  \item<+-> Main complexity
    ...
    \item One execution of conditional uses one execution from each branch
    \end{itemize}    
