...
\cite{alglave-git-alternate,armed-cats}, which is very close to our model;
see also \cite[\textsection B2.3.6]{arm-reference-manual}.

\subsection{Down-Grading Acquires ($\xDGA$)}
\label{sec:internal}
...
downgrades a read from $\mRA$ to $\mRLX$ when it read can be fulfilled by a
relaxed write in the same thread.
\end{example}

To model such \emph{downgraded acquires}, we use the uninterpreted symbols
$\Dx{\aLoc}$.
...
\begin{definition}[$\xCO$/$\xRASC$/$\xDGA$]
  \label{def:pomsets-down}
  Update \refdef{def:pomsets-ra} to:
  \begin{enumerate}
  \item[\ref{S4})]
    $\aTr{\bEvs}{\bForm}$ implies $(\Qw{\aLoc}\limplies\aExp{\neq}\aVal)\land \DS{\aLoc}{\amode}\bForm[\aExp/\aLoc]$,
  \item[\ref{S5})]
    $\aTr{\cEvs}{\bForm}$ implies $\lnot\Qw{\aLoc}\land \DS{\aLoc}{\amode}\bForm[\aExp/\aLoc]$.
  \item[\ref{L5})]
    $\aTr{\cEvs}{\bForm}$ implies $\lnot\Qx{\aLoc} \land \DL{\aLoc}{\amode} \land((\aVal{=}\aReg\lor\aLoc{=}\aReg)\limplies\bForm)$,
  \item[\ref{L6})]
...
\armeight{} allows some counterintuitive results for SC access.  In the
following execution we elide the initializing write $\DWP{y}{0}$.
\begin{gather*}
...
\begin{gather*}
  \begin{gathered}
  x\GETS0\SEMI 
  x\GETS 1\SEMI y^\mRA\GETS1 \PAR r\GETS y\SEMI \IF{r}\THEN s\GETS x\FI
  \\[-.4ex]
  \hbox{\begin{tikzinline}[node distance=1.5em]
      \event{wx0}{\DW{x}{0}}{}
...
We repeat all of the rules for preconditions and transformers,
...
\begin{definition}[$\xCO$/$\xRASC$/$\xDGA$/$\xRRD$]
  \label{def:pomsets-down-rr}
  Update \refdef{def:pomsets-trans} to:
  \begin{enumerate}
  \item[\ref{S3})]
