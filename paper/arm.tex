...
We discuss \armeight{} using \emph{external completion} (\EC{})
...
\begin{example}
  Should not wipe out local state only when there is a release-acquire pair,
  as in the definition of \jjr{}, which allows the following execution.  Even
  though there is a blocker $\DWP{x}{3}$, here we are using $\DWP{x}{0}$ to
  justify the lack of dependence $\DRP{x}{4}\xpo\DWP{w}{4}$.
  \begin{gather*}
    \PW{x}{0}\SEMI
    \IF{x}\THEN\PW{y}{2}\FI\SEMI
    \IF{\PR[\mACQ]{z}{}}\THEN
      \PR{x}{s}\SEMI
      \IF{s\;\mathsf{even}}\THEN\PW{w}{4}\FI
    \FI
    \PAR
    \PW{x}{2}\SEMI
    \IF{\PR[\mACQ]{y}{}}\THEN
    ...
    \hbox{\begin{tikzinline}[node distance=1.5em]
        \event{2a}{\DW{x}{0}}{}
        \event{2b}{\DR{x}{2}}{right=of 2a}
        \event{2c}{\DW{y}{1}}{right=of 2b}
        \raevent{2d}{\DR[\mACQ]{z}{1}}{right=of 2c}
        \event{2e}{\DR{x}{4}}{right=of 2d}
        \event{2f}{\DW{w}{4}}{right=of 2e}
        %
        \event{3a}{\DW{x}{2}}{below=of 2a}
        \raevent{3b}{\DR[\mACQ]{y}{1}}{right=of 3a}
        \event{3c}{\DW{x}{3}}{right=of 3b}
        \raevent{3d}{\DW[\mREL]{z}{1}}{right=of 3c}
        \event{3e}{\DR{w}{4}}{right=of 3d}
        \event{3f}{\DW{x}{4}}{right=of 3e}
        \wk{2a}{2b}
        \po{2b}{2c}
        \sync{2d}{2e}
        \sync[out=20,in=160]{2d}{2f}
        \sync{3b}{3c}
        \sync{3c}{3d}
        \po{3e}{3f}
        \rf{3a}{2b}
        ...
        \rf{2f}{3e}
        \wk{2a}{3a}
        %\wk[out=-20,in=-160]{3c}{3f}
        %\wk[out=20,in=160]{2b}{2e}
      \end{tikzinline}}
  \end{gather*}
  (The read-clobbering semantics from \textsection\ref{sec:substitutions}
  does not change this.)
\end{example}

It does seem to me like a non-local blocker for program-order must include a
read of $x$ or a release (to get order from the write) and an acquire (to get
the order to the read).

What if an acquire preceded by a release or non-local read on $x$ wiped the
local state of $x$?  The problem with this kind of solution (and P6 in ooplsa
specifically) is that it destroys roach-motel.
...
\begin{example}
  ...
  $\DRP[\mACQ]{x}{2}\xsync\DWP{y}{1}$, because $\DRP[\mACQ]{x}{2}$ is fulfilled
...
\begin{definition}
  ...
  \begin{align*}
    ...
    \begin{cases}
      ...
      \\
      [\TRUE/\Dx{\aLoc}][\FALSE/\D] & \textotherwise
    \end{cases}
  \end{align*}
  % \begin{align*}
...
\begin{example}
  ...
  \begin{gather*}
    \begin{gathered}
      \PW{x}{0}\SEMI \PW{x}{1}\SEMI \PW[\mREL]{y}{1}
      \PAR
      \PR{y}{r}\SEMI \IF{r}\THEN \PR{x}{s}\FI
      \\[-.4ex]
      \hbox{\begin{tikzinline}[node distance=1.5em]
          \event{wx0}{\DW{x}{0}}{}
...
\begin{example}
  ...
  \begin{align*}
    \begin{gathered}[t]
      ...
      \\
      \hbox{\begin{tikzinline}[node distance=0.5em and .5em]
          \raevent{b}{\QL{y}{\mRA}\mid\DR[\mACQ]{y}{1}}{}
  ...
  \begin{align*}
    \begin{gathered}[t]
      ...
      % \\
      \hbox{\begin{tikzinline}[node distance=0.5em and 1.5em]
          \raevent{b}{\TRUE\mid\DR[\mACQ]{y}{1}}{}
  ...
  preceded by a relaxed write to $y$, allowing $\DRP[\mACQ]{y}{1}$ to be
