...
We use the uninterpreted symbols $\Dx{\aLoc}$.
...
\begin{definition}[$\xCO$/$\xRASC$/$\xDGA$]
  ...
  Update Def.~\ref{def:pomsets-ra} to:
  \begin{enumerate}
  \item[\ref{S4})]
...
\subsection{Read-Read dependencies ($\xRRD$)}

We use the uninterpreted symbol $\RW$.
...
Only \ref{L5} and \ref{T3} are changed from Def.~\ref{def:pomsets-down}.
