...
program order ($\rpox$) and phantom events ($\fmrg{}$).  The construction is
discussed in \textsection\ref{sec:c11}.
\begin{lemma}
  Any $\aPS$ in the image of $\frf{\semrr{}}$ is complete iff
  ...
  \item internal fulfillment:
    $(\exists\bEv'\mid\fmrg{\bEv'}=\bEv)$
    $(\exists\aEv'\mid\fmrg{\aEv'}=\aEv)$
    $\bEv'\xpox\aEv'$ and $(\not\exists\cEv')$
