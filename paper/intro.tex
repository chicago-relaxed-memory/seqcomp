...
\item computed dependencies using non-compositional operational models over
  ...
  models validate many compiler optimizations, but
...
dependencies for \cXI, targeting the Intermediate Memory Model (\IMM)
\cite{DBLP:journals/pacmpl/PodkopaevLV19}. \citet{DBLP:journals/pacmpl/JagadeesanJR20} defined
Pomsets\footnote{A pomset is a labeled partial order.} with Preconditions (\PwP), which use preconditions and logic to
calculate dependencies for a Java-like language targeting {multicopy-atomic} (\mca{}) hardware, such as
...
The model has been formalized in Coq.  We have formally verified that the
...
\begin{gather*}
  ...
  \hbox{\begin{tikzinline}[node distance=1.5em]
    ...
    \end{tikzinline}}
\end{gather*}
In general, there will be many such executions, reflecting different
...
and includes control and data dependencies ($\xpo$), to avoid
...
\begin{gather*}
  ...
  \hbox{\begin{tikzinline}[node distance=1.5em]
    ...
    \end{tikzinline}}
\end{gather*}
In this diagram, $(s {<} 1)$ is a control dependency and $(r{*}s){=}0$ is a data dependency. Preconditions are discharged by being ordered after a read (we assume the
...
penalties (see also \cite{10.1145/3462206}).  In this paper, we take the opposite approach.  We have
