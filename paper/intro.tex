...
$\fwp{\aCmd}{\bForm}$ for loop-free code below (where $\aReg$ ranges over thread-local \emph{registers}
and $\aExp$ ranges over side-effect free \emph{expressions}). 
\begin{multicols}{2}
...
context.  In a concurrent context, however, generalizing
\ref{wp-let} from thread-local registers to cross-thread memory locations
is unsound! \ref{wp-let} is sound because a read from a thread-local
register must be fulfilled by a preceding write
in the same thread.  In a concurrent context, aread from a cross-thread
memory location may be fulfilled by a write
in another thread.
