...
\begin{itemize}
\item didn't bother tracking dependencies, allowing ``thin air'' executions
...
\item tracked dependencies conservatively, using syntax, requiring inefficient
  ...
  \cite{DBLP:conf/pldi/LahavVKHD17,Boehm:2014:OGA:2618128.2618134,DBLP:conf/oopsla/VafeiadisN13,DBLP:journals/corr/abs-1804-04214}---while
  this may be fine for C, it's a non-started for safe languages like Java,
  which lack C's ``plain'' access,
\item computed dependencies using non-compositional operational models over
  alternate worlds \cite{Manson:2005:JMM:1047659.1040336,
    ...
    DBLP:conf/popl/KangHLVD17,DBLP:journals/pacmpl/ChakrabortyV19,DBLP:conf/pldi/LeeCPCHLV20,promising-ldrf}---as
  discussed in \textsection\ref{sec:promising}, these models fail to validate
  temporal safety properties.
  %JMM, promising semantics, weakestMO, etc
\end{itemize}
...
calculate dependencies for a Java-like language on \mca{} hardware, such as
...
In this paper, we show that \PwP{} can be extended with \emph{families of
  predicate transformers} to calculate sequential dependencies in a way that
...
One of the main difficulties is to find a definition that is associative:
the denotation of $((\aCmd_1\SEMI \aCmd_2)\SEMI \aCmd_3)$ should be the same as 
the denotation of $(\aCmd_1\SEMI (\aCmd_2\SEMI \aCmd_3))$.

We have layered the definitions, both to manage complexity and to make clear
that the main contribution of this paper is broadly applicable.  After an
overview, we define sequential dependencies in \textsection\ref{sec:model}.
The next two sections add concurrency.  In \textsection\ref{sec:mca}, we
define \PwTmca{}, which models Java for \mca{} hardware, similar to that of
\citet{DBLP:journals/pacmpl/JagadeesanJR20}.  In \textsection\ref{sec:c11},
we define \PwTc{}, which models \cXI, adapting the approach of
\citet{DBLP:conf/esop/PaviottiCPWOB20}.

We describe two variants of \PwTmca{}.  The first has a nicer semantics; the
second has a more efficient lowering for acquiring reads on \armeight{}.
Both variants have optimal lowering for relaxed access.

\todo{flesh this out}
\begin{itemize}
\item We provide a tool to execute litmus tests for both models.
\item \textsection\ref{sec:drf} We prove \drfsc{} for \PwTmca{}; \MRD{} inherits the result from \rcXI.
